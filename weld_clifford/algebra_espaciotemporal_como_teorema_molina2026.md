...
La mecánica clásica y la electrodinámica libre aparecen como límites estructurales. El teorema es
condicional: presupone que un dominio admita $S,\mathbf{A},\mathbf{I},\mathbf{R}$ como atributos de
grado 0/grado 1 en primer lugar, premisa que este artículo no establece para ningún dominio específico (§8.4).
...
Este artículo forma parte de un programa más amplio, el Gamma Space Framework (GSF; Molina 2025), cuyo
objeto central es una matriz de configuración real $4\times4$, $\Gamma \in M_4(\mathbb{R})$. El presente
...
gradiente matricial. El término **unidad dinámica operativa (UDO)**, usado a lo largo de este artículo,
es autocontenido y no requiere conocimiento previo del GSF.
...
en la convención de Hestenes). La elección de firma no es una mera convención:
...
$\mathrm{Cl}_{3,1}$, de forma única, por un argumento de tipo Schur (compatibilidad de representación) que
...
mantiene deliberadamente fuera de aquí (ver §8.4). La palabra "único" en A1 debe leerse como "único al
nivel del contenedor, probado; abierto al nivel de instancia" hasta que ese trabajo compañero madure.
Esto no debilita a A1; aplica la misma disciplina que §8.4: decir exactamente qué está probado, y no
dejar que una palabra fuerte en un axioma implique más de lo que el teorema entrega.
...
matriz, lo que hace fácil leer esto como que la separación Fuerza/Campo de A2 se usó una vez, en §2, y
luego se abandonó. No es así: cada término de la EOM actúa sobre ambos sectores simultáneamente, porque
...
bivector; es una estructura algebraica distinta. Esta rama no se desarrolla en este artículo.
...
la identificación como un cuarto postulado, ya que la derivación lo necesita y antes se usaba sin
declararse:
...
métrica de coordenadas). A3′ es, por tanto, decisivo precisamente en la costura nombrada en el Lema 3
y retomada en §8.4: sin él, el Lema 4 restringe solo el espacio de coordenadas
...
exactamente cuando la condición de suavidad de A3 expone una dirección independiente que
...
teorema cerrado de este artículo y se marca como tal en §8.4.
...
solo que $\gamma_0$ es el generador algebraico conjugado a $\partial_\tau$ en la factorización de
...
de $\mathrm{Cl}_{3,1}$ consta de representaciones irreducibles no isomorfas entre sí de
...
no enfrenta y que otras rutas de firma emergente (§8.3) no hacen; es específica a la exigencia de que
...
### 8.2 Corolario: el resultado de la firma agota las alternativas

El Lema 4 selecciona $(3,1)$. El mismo argumento, corrido sobre el espacio completo de símbolos
candidatos en vez de solo el que A3 produce, *clasifica* las alternativas en lugar de meramente
excluirlas, lo cual fortalece el teorema sin costo adicional.
...
Dos consecuencias siguen para la lectura del Lema 4. Primero, la unicidad de $(3,1)$ no es una afirmación sobre
...
### 8.3 Relación con los enfoques de firma emergente
...
independientes, y la presente descansa en la premisa más económica: la buena postura de una evolución
real, en vez de un álgebra split elegida.
...
- Establece, para un dominio dado, la **existencia** de una cuádrupla SAIR en primer lugar (segunda
  ...
  grado 1, con Fuerza y Campo genuinamente inherentes, es empírico, no algebraico, y no se establece
  aquí para ningún dominio. Esto es lógicamente previo a, e independiente de, la pregunta de unicidad
  de arriba; el teorema del contenedor es
  condicional a la existencia y no prueba nada sobre ella

Dos huecos estructurales en el conjunto de axiomas se abordan directamente a continuación, cada uno con
una resolución precisa en vez de una admisión desnuda.

**(a) El espacio de atributos y el espacio de coordenadas, identificados sin argumento: Postulado A3′.**
A1 y el Lema 1 dan un espacio de atributos $\mathbb{R}^3$ *interno* generado por
...
laplaciano. El paso que identifica los dos espacios queda nombrado explícitamente, inmediatamente
después del Lema 3, como **Postulado A3′**. La identificación no es derivable de A1–A3 tal como están
enunciados (una UDO podría en principio llevar atributos internos que no coincidan con sus coordenadas
de propagación), así que se enuncia como una cuarta premisa independiente, en el mismo pie que la
irreducibilidad de A1 (Observación 2.1). El alcance del teorema siempre fue condicional a esta
identificación; nombrarla como A3′ permite al lector ver esa condición y evaluarla, en vez de
encontrarla absorbida en silencio dentro de «las direcciones de atributos espaciales de A1» en el
Lema 3.

**(b) Dos objetos distintos, ambos llamados «la firma»: Corolario 4.2.** El Lema 4 y el Corolario 4.1
conciernen a la inercia del *símbolo principal* sobre
...
que también lleva una inercia. Estas no necesitan coincidir en general: son $4$-espacios distintos, y la
construcción de la Gram admite dos lecturas (§2). El Corolario 4.2 enuncia la condición exacta bajo la
cual sí coinciden: la Gram hereda la firma $(3,1)$ del símbolo, por la ley de inercia de Sylvester,
precisamente cuando se construye como una congruencia $V^{\mathsf T}\eta V$ con $V$ invertible. Bajo la
construcción por-casillero alternativa, también licenciada por A2, y la que se usa siempre que un
atributo no lleva estructura de Minkowski propia, tal herencia no se cumple, y la firma varía sobre
todas las clases admisibles, coincidiendo con lo que observa un artículo compañero al leer regímenes
dinámicos a partir de la Gram. Las dos lecturas de la Definición en §2 simplemente no se habían
distinguido antes.

Los residuos que quedan abiertos tras P1/P2/P3 son los tres axiomas originales A1, A2, A3, junto con el
Postulado A3′ recién nombrado. El hueco (a) se resuelve declarando A3′ como una premisa, cuya verdad
para un dominio físico dado es una pregunta de modelado, no un teorema. El hueco (b) se resuelve por
completo: el Corolario 4.2 es un teorema con una hipótesis verificable, no un problema abierto. El
número de axiomas crece de tres a cuatro, pero ninguna premisa entra en la derivación sin declararse, y
las dos nociones de «firma» ya no pueden confundirse en silencio.

### 8.5 Trabajo relacionado
...
$\mathrm{Cl}_{3,1}$ es la representación forzada». Este último paso es condicional a que la unidad
tenga, en primer lugar, una cuádrupla SAIR (§2, §8.4); el teorema fuerza el álgebra, no la existencia
de los atributos sobre los que actúa.
