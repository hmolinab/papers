...
Twelve composition operations, so far catalogued case by case across different domains, close into five primitives and a single algebraic identity (the Schur block-determinant formula) plus **two facts already established** in earlier program work (the extensivity of co-presence and the Lyapunov monotonicity of the potential $P$). That closure (Theorem 2, with the complete entropy balance derived in §6) is the paper's central result. Around it, two structural extensions that were not part of the original plan: the determinant-sign classification used throughout the paper turns out to be, for any dimension, the coarsest possible partition of a symmetric matrix's inertia (Corollary 8.1, a fact of real algebraic geometry), and no conjugation-invariant potential can break that classification between sub-sectors sharing phase (Theorem 9). A third, more incipient line proposes that the full algebra is an entropy-enriched category, documented in Appendix A as work in progress and not in the main body, because its status is [A] while this body stays [D]/[V].

**Placement.** A result in algebra and dynamical systems, in `math-ph` / `nlin.AO` (Adaptation and Self-Organizing Systems); not particle physics. The examples in §11 (Coulomb/Lorentz, ideal gas, chemical bonding) are illustrative correspondences 〔SC〕 showing the algebra's applicability to already-known cases.

**Dimension.** Unless stated otherwise (Theorems 8, 9, and Corollary 8.1, formulated for arbitrary $n$), $n=4$ in Theorems 1–7 is a choice specific to the SAIR construction ($\Gamma\in M_4(\mathbb R)$), not a general result about the dimension. Where the argument is dimension-agnostic (Haynsworth, the det-sign tripartition, Theorem 9), the statement says so explicitly with $n$ free.

---

## 2. Related work

The relevant literature comprises three areas, none of which this paper claims novelty in separately: the contribution is the specific synthesis (Schur + Haynsworth inertia + compositional thermodynamics) over the configuration object $\Gamma$, applying known tools to a concrete object.
...
**Schur complements in Gaussian graphical models.** The central identity (Proposition 1, and Theorem 1's admissibility) is, at its core, the standard fact that the Schur complement of a Gaussian precision matrix is exactly that distribution's marginalization, textbook material in the graphical-models literature (Lauritzen, *Graphical Models*, and its descendants in structured Gaussian inference). The contribution here is applying it to a physical configuration object ($\Gamma$), with a thermodynamic interpretation ($\rho=-\log|\det\Gamma|$ as structural entropy) absent from the purely statistical context; the identity itself is textbook.
...
The axioms and classification that follow (§5, (A4); §10) use "identity preserved/lost." We define distinguishability here before using it.
...
> **▣ 〔THM〕 Lemma 1 (Uniqueness of the $\Omega$ closure). [D]** Among maps that reduce cardinality by integrating out a subspace under (A1)-(A2), the Schur complement is the only one that preserves the retained block's effective description. No hypothesis on $P$ is required.
>
> *Proof.* $\Gamma_{\mathrm{joint}}$ is a Gram matrix by construction (A2): writing the attributes of $A$ and $B$ as the columns of $V_A,V_B$ in the ambient space, $\Gamma_{\mathrm{joint}}=V^{\mathsf T}V$ with $V=[V_A\mid V_B]$, so the coupling block is $C_{AB}=V_A^{\mathsf T}V_B$, the relative orientation of the two attribute sets. Eliminating $B$ means passing to the quotient by $\operatorname{span}(V_B)$ with the induced metric, and the Gram of the retained attributes in that quotient is
> $$(V_A-P_{V_B}V_A)^{\mathsf T}(V_A-P_{V_B}V_A)=\Gamma_A-C_{AB}\Gamma_B^{-1}C_{AB}^{\mathsf T},$$
> with $P_{V_B}=V_B\Gamma_B^{-1}V_B^{\mathsf T}$ the orthogonal projector onto $\operatorname{span}(V_B)$: exactly the Schur complement. Equivalently, in variational form, the Schur complement is the partial minimization of the quadratic form carried by the Gram, $\min_{c_B}c^{\mathsf T}\Gamma_{\mathrm{joint}}c=c_A^{\mathsf T}\Omega\,c_A$, i.e. the eliminated block relaxed to its optimum given the retained one. Uniqueness: the orthogonal projector onto a subspace is the unique self-adjoint idempotent onto it, so no other reduction yields the quotient Gram; the naive projection $\Gamma_{kk}$ discards $C$'s back-reaction and differs (verified). $\blacksquare$ Both identities verified to machine precision over 500 random Gram configurations (`lema1_ruta_geometrica.py`).
>
> *Remark (why not the Gaussian route).* Reading $\Gamma_{\mathrm{joint}}$ as a Gaussian precision matrix and invoking standard Gaussian marginalization gives the same Schur complement, but only for quadratic $P$, and at the cost of postulating a probability distribution the framework does not otherwise use: elsewhere $\Gamma$ evolves under a deterministic flow, not as the precision of a distribution. The two routes coincide because for quadratic forms marginalizing and minimizing agree. The geometric route above is stated instead because it answers the question this algebra actually asks (what is the Gram of what survives, a structural fact, the same notion used by $\rho=-\log|\det\Gamma|$, by admissibility, and by Haynsworth inertia) and because it carries no hypothesis on $P$.

> **▣ 〔THM〕 Theorem 2 (Closure of the primitive basis). [D]** Under (A1)-(A4), every admissible compositional operation factors, up to composition, into five primitives: **JOIN** ($\oplus$, form the set), **COUPLE/DECOUPLE** (fix $C_{AB}\neq0/=0$), **$\Omega$** (marginalize, Schur complement), **COPY** ($\Gamma_B^{(0)}\leftarrow\Gamma_A$), **RELAX** (gradient flow to $\arg\min P$).
>
...
>
> **What the theorem does and does not require of $P$.** An earlier version of this result was stated as conditional on quadratic $P$, inherited from proving Lemma 1 through Gaussian marginalization. With Lemma 1 proved geometrically instead, no axis of the closure argument constrains the form of $P$: the cardinality axis rests on the Gram structure of $\Gamma$ (A2), the coupling axis on (A2) alone, and the block-content axis requires only that $P$ admit a gradient flow toward a minimum, not that it be quadratic. In particular the theorem now covers the program's actual potential, whose term $\mu\det\Gamma$ is of degree 4 in the entries of $\Gamma$ (a $4\times4$ matrix), not degree 3. (The word "cubic" belongs to a different object: the coefficient of $\xi^3$ in the one-dimensional normal form obtained by projecting $P$ onto a soft mode $\xi$, where the companion paper on the determinant as the source of that coefficient shows $D^3\det$, evaluated at the equilibrium, is linear in $\Gamma_*$ because $\det$ has degree 4.)
>
> **What remains outside the theorem.** The closure proved here is algebraic and kinematic: it classifies which transformations of $\Gamma_{\mathrm{joint}}$ are admissible, not how the flow behaves globally. Under a non-quadratic $P$, a small perturbation to the potential can qualitatively alter the phase portrait (bifurcations, new fixed points, loss of $\arg\min P$'s uniqueness) without affecting any step above, since none of them reads $P$'s shape. Whether the *dynamics* built on this algebra is structurally stable under the program's actual $P$ is a separate question, belonging to dynamical systems rather than to compositional algebra, and lies outside this paper by design.
...
**Limits of the closure theorem.** The closure proof factors along the three degrees of freedom that (A1)-(A2) allow a collection-state (cardinality, coupling, block content) rather than enumerating known operators and ruling out the rest by brute force. A sixth primitive would have to act on a **fourth** degree of freedom; by (A1), $O$ acts only on $\Gamma_{\mathrm{joint}}$, so that fourth axis does not exist within the joint representation as defined: the complete set is derived from the object's structure, not obtained by filtering candidates.

This has a limit worth marking precisely. A strong-sense universality theorem (a Cayley-type representation, or freeness of the generated monoid) would prove that *no* alternative representation of "CU composition" can reveal a fourth axis. The result proved here is relative to the stated representation: within **this** specific representation ($N$ sub-CUs, blocks $\Gamma_i$, couplings $C_{ij}$) there is no fourth axis.

---

## 5bis. Additional operations outside the original catalogue

Theorem 2 closes the **catalogue**: the nine phenomenological operations known at the time it was proved factor into five primitives. Later work, applying this same algebra to continuous field configurations (a boundary declared in §1, outside this paper), found **two more operations of repeated, necessary use** that were not in the original list. Their absence does not break Theorem 2 (which is correct about the catalogue it closed), but it does show that "closed" and "complete" are different claims: completeness of the primitive basis was never proved, only closure of the catalogue known at the time (noted as a limit in §5 above: the proof closes against the three axes of *this* representation, not against any future operation on the same object).
...
Verified by finite differences (error $<10^{-8}$, `rho_P_exact_relation.py`). The result: **$\rho$ and $P$ are distinct entropy currencies**; the coefficient of variation of their ratio over random samples is $\approx1.45$, not zero. There is no hidden $\rho\propto P$ identity; there are two complementary conservation laws over the same flow.

Theorem 7 **proves** that no linear identity ties $\rho$ and $P$ for this flow, with the same rigor as any other [D] result in this paper; non-proportionality is proved, not pending discovery of a missing unifying principle. $\rho$ (via Schur, kinematic) and $P$ (via Lyapunov, dynamic) measure distinct aspects of the same flow, as energy and entropy do in classical thermodynamics: two laws coupled through free energy, each with its own dynamics.
...
Found by applying the algebra outside this paper's usual domains: mapping real tabulated physical data (electronegativity, bond dissociation energy) for a hydrogen and an oxygen atom onto SAIR→Γ objects, and composing them via Union+Ω to see whether the algebra distinguishes the OH radical from H₂O (`brainstorming/physics/uoc_st_toroide/h2o_prueba_fuego_sair_gamma.md`).
...
Theorem 10 left its converse as a merely verified fact: that eliminating a partner *with* a bivector injects a bivector into the survivor, without saying *why* or *how much*. The theorem below, from `part1/07_compositional_operations.md` §7.3.3, closes that question; it is incorporated here, with no change to the statement, and the update propagates to §10, §13, and Appendix B.
...
**Antisymmetric component after elimination.** Theorem 11's clean decomposition does not propagate through $\Omega$: $\Gamma_a(\Omega)$ is, in general, not a simple function of $D_{\mathrm{eff}}$ alone (numerically verified: $K_A^{-1}$ entangles the reciprocal and non-reciprocal channels during elimination, because inverting a non-symmetric matrix does not preserve the symmetric/antisymmetric split of what it multiplies). This is exactly where Theorem 8 (a statement about $\Omega$, not about $\Gamma_{\mathrm{joint}}$) stops generalizing: not at composition (Theorem 11 is exact there), specifically at elimination. For that regime the case-by-case linear composition `models/sair/core/state_system.py` (`StateSystem`) provides the answer, not a closed form. Verified: `models/calcs/brainstorming/ds/algebra_bloques_no_simetricos/03_teorema_reciprocidad.py`.
...
5. **Item 3's open question: partial answer via Theorem 9 (§9). [D]+[V]** Item 3 left open whether $P$'s dynamical flow selects finer structure within Haynsworth's coarse classification. The question tested: can a conjugation-invariant potential (the class $P(\Gamma)=\|\Gamma\|_F^2+\mu\det\Gamma$ belongs to) distinguish sub-sectors sharing phase? **Result: no, never, for this class of potentials** (Theorem 9, verified at $n=3,4,6$). This does not close item 3's question; it sharpens it: any dynamical selection of fine structure would require explicitly breaking conjugation invariance, an ingredient absent from $P$ as used in this paper.
6. **Composition applied to a real chemical case (H₂O), see §9bis and §11.4. [D]+[V]/negative** The algebra was tested against a concrete chemical fact (H₂O stable, OH unstable) with real tabulated data (electronegativity, bond energy). The direct attempt **failed** at physically realistic parameters (§11.4), but the attempt surfaced Theorem 10 (§9bis), a structural fact about $\Omega$-elimination that had not been sought. Diagnosis (`h2o_reversa_algebra_vs_sair.md`): the failure is the SAIR→Γ mapping's (units/normalization with no fixing physical principle), not the algebraic composition's; the same machinery (Union+$\Omega$) that gives Theorems 1–9 unchanged.

---

## 13. Status of results

| Result | Register | Status |
|---|---|---|
| Coupling admissibility bound (Thm. 1) | 〔THM〕 | [D] |
| Closure of the primitive basis (Thm. 2) | 〔THM〕 | [D]: unconditional in $P$ since Lemma 1's geometric proof (§5) |
...
| OH vs. H₂O, concrete application of Thm. 10 with real data (§11.4) | 〔SC〕 | [D] (theorem) + **negative** (symmetric channel does not reproduce real-scale stability; failure traced to the SAIR mapping) |
...
**Named frontiers:** (i) the algebra is exact for any $P$ (Lemma 1, §5), but the *dynamics* built on it under a non-quadratic $P$ (structural stability of the flow, bifurcations) is not addressed here; (ii) the exact relation between the two entropy currencies remains two complementary laws, not one identity; (iii) the kinetic layer (rates, barriers, catalysis) is out of scope: the algebra is kinematic, not rate dynamics; (iv) completeness of *generation* of the primitive basis (§5bis) remains open, distinct from the catalogue completeness Theorem 2 already closes. The algebra as a categorical structure (Appendix A) is a fifth frontier, treated separately for its [A] status.

---

## 14. Discussion and closing

The main results are Theorem 2 (closure of the primitive basis), the complete entropy balance (§6), and the work/spontaneity bounds (§7), all [D]+[V] with no [A]-marked exceptions in the main body. The two extensions beyond the originally admissible regime (Haynsworth, §8; Theorem 9's structural negative, §9) were not part of the original plan; they arose from investigating why a naive cascade-sign conjecture died (§12, item 3): the determinant-sign classification, used throughout this program from the start, is the coarsest possible partition of inertia, and no conjugation-invariant potential can break it dynamically between sub-sectors sharing phase. Both are, in retrospect, direct consequences of taking seriously what $\Gamma_s$ actually is as an algebraic object.

**Open problems.** Four named frontiers (§13) mark where what is proved ends: the dynamics of the flow under a non-quadratic $P$ (the algebra itself is unconditional), the relation between the two entropy currencies, the full kinetic layer, and the completeness of *generation* of the primitive basis (§5bis), distinct from the catalogue completeness Theorem 2 does close: two primitives found on the continuous-field frontier (Self-Coupling, Slaving) after proving that theorem show the primitive list may not have been complete from the start, and there is still no argument ruling out a seventh. Added to these is the question closing Theorem 9 (what ingredient, explicitly not conjugation-invariant, would be needed to break the blindness between sub-sectors), left unattacked by design: it is a question of concrete physics (what that ingredient is in a real system), not of compositional algebra in the abstract. Appendix A's categorical line is a sixth frontier, with real progress already made but not closed.

A subsequent paper extends the compositional machinery to continuous systems: fluids and fields, where $\Gamma$ stops being a finite matrix and composition becomes an operation on fields coupled in space, over an infinite-dimensional configuration space. If this paper's algebra is, as argued here, a consequence of the block structure and the Schur identity rather than of $\Gamma$'s specific finite dimension, it should survive that step, subject to verification. Two further routes are flagged: (i) finishing the formalization of Appendix A's categorical bridge, in particular the cocycle law of the $\Delta_2$ 2-cell and its assembly into the standard axioms of a lax 2-category; (ii) investigating whether the rest of the fine inertia signature (beyond the sign tripartition, §8), e.g. whether $P$'s gradient flow preferentially stabilizes specific signatures like $(2,2,0)$ over $(3,1,0)$ at $n=4$, has dynamical content, the question Corollary 8.1 leaves explicitly open.

---

## Appendix A. Work in progress: the algebra as a category enriched over $\Delta\rho$ [A]

*This section stays outside the main body (§1–§14) because its status is [A] overall, not [D]/[V]; it is documented here rather than omitted, but it is not part of what this paper certifies as closed.*

Working through the five primitives (§5) in detail surfaced something absent from both the standard categorical treatments of composition (Baez-Fong, Span/Cospan, PROPs, operads) and from classical process thermodynamics: an **explicit kinematics**. Moving a continuous physical parameter does not jump between discrete operations; it flows continuously from Union to Coupling to Fusion, with $\Delta\rho$ varying smoothly throughout. Ordinary (Set-enriched) categories have no native way to carry that kinematics: their morphisms are arrows without magnitude.
